(function ($) {
	...
	window.taxonomyAdminFilterApply = function($elem){
    ...
    var taxonomy = product_cat_tax_id.substring(9);
    var tax_options = options.taxonomies[taxonomy];
    var lang = options.lang;
		$('body').on('DOMSubtreeModified', '#' + taxonomy + '-all', function(){
			setTimeout(function(){
				$('#' + taxonomy + '-all > ul li:not([data-selected])').each(function(){
					var item_text = $.trim($('>label', this).text());
					var item_id = $('>label>input[type=checkbox]', this).val();
					$(this).attr({
						id: taxonomy + '-' + item_id,
						'data-contentlc': stripDiacritics(item_text).toLowerCase(),
						'data-content': stripDiacritics(item_text),
						'data-cnt': 1,
						'data-count': 0,
						'data-selected': $('>label>input[type=checkbox]', this).is(':checked') ? 1 : 0
					});
    ...
    var $checkbox_container = $('#' + taxonomy + '-all', $this);
    var $checkbox_ul = $('>ul', $checkbox_container);
    var $checkbox_pop = $('#' + taxonomy + '-pop', $this);

    $checkbox_ul.on('change', 'input[type=checkbox]', function (event) {
      var $this = $(this);
      var $parent_li = $this.closest('li');
      if (tax_options.select2 && !$parent_li.hasClass('popular-category')) {
        if (!this.checked) {
          $parent_li.remove();
        }
      } else {
      ...
      $('#in-popular-' + taxonomy + '-' + this.value).prop('checked', this.checked);
      calculateSelected();
    });
    var last_clicked = -1;
    ...
    $checkbox_pop.on('change', 'input[type=checkbox]', function () {
      var $checkbox = $('#in-' + taxonomy + '-' + this.value, $checkbox_ul);
      $checkbox.closest('li').attr('data-selected', this.checked ? '1' : '0');
      calculateSelected();
      if (tax_options.select2) {
        calculateTotal(total_visible + (this.checked ? 1 : -1));
      }
    });
    ...
    if (tax_options.select2)
    {
      ...
      $select2_select.select2({
        ...
        if (selecting)
        {
          ...
          if ($checkbox.length) {
            var $parent_li = $checkbox.closest('li');
            if ($parent_li.hasClass('popular-category')) {
              if ($checkbox.is(':checked')) {
          ...
          if ($parent_li.hasClass('popular-category')) {
            $('#in-popular-' + taxonomy + '-' + item.id).prop('checked', false);
          }
	...
	var options = taxonomy_admin_filter_options;
  $(options.selector).each(function(){taxonomyAdminFilterApply(this)});
})(jQuery);