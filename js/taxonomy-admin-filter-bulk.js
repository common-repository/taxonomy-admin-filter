(function ($) {
  ...
  $(document).ready(function () {
    $(options.selector).each(function () {
      var $this = $(this);
      var tax_matches = $this.attr('class').match(/cat-checklist[\s+](\w+)-checklist/);
      if (!tax_matches || !tax_matches.length) {
        return;
      }
      var taxonomy = tax_matches[1];
      var $container = $this.closest('tr');
      var quick_edit = $container.hasClass('quick-edit-row');
      var bulk_edit = $container.hasClass('bulk-edit-row');
      var container_class;
      if (quick_edit) {
        container_class = 'quick-edit-row';
      }
      if (bulk_edit) {
        container_class = 'bulk-edit-row';
      }
      var container_id = $container.attr('id');
      $this.addClass('taxonomy-admin-filter filtering-' + container_id);
      var $product_cat_tax = $this;
      var product_cat_tax_id = taxonomy + '-' + container_id;
      ...
      $('body').on('change', '.cat-checklist.' + taxonomy + '-checklist input[type=checkbox]', function (event) {
        var $this = $(this);
      ...
      var last_clicked = -1;
      $('body').on('click', 'table.wp-list-table .' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist input[type=checkbox]', function (event) {
        var current_index = $(this).closest('li').index();
        if (event.shiftKey) {
          var $checkbox_ul = $('.' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist');
          var gt = Math.min(last_clicked, current_index) - 1;
          var lt = Math.max(last_clicked, current_index) + 1;
          $('.' + container_class + ':visible li:lt(' + lt + '):gt(' + gt + ')>label:visible input[type=checkbox]' + (this.checked ? ':not(:checked)' : ':checked'), $checkbox_ul).prop('checked', this.checked).trigger('change');
        }
      ...
      if (tax_options.select2)
      {
        ...
        var $select2_select_clone = $select2_select.clone();
        $filter_container.append($select2_select);
        $('body').on('click', '.editinline, #doaction', function () {
          if (this.id === 'doaction')
          {
            ...
            if (quick_edit)
            {
              ...
              var post_id = $closest_tr.attr('id').substring(5);
              var post_category = $('#inline_' + post_id + ' .post_category#' + taxonomy + '_' + post_id);
              var tax_selected = post_category.text().split(',');
              if (tax_selected.length)
              {
                $.ajax({
                  ...
                  delay: 500,
                  cache: true,
                  data: {// page is the one-based page number tracked by Select2
                    action: 'taxonomy_admin_filter',
                    taxonomy: taxonomy,
                    post_id: post_id,
          ...
          function start_bulk_check() {
            var $select2_select = $('#' + product_cat_tax_id + '-select2');
            var $table = $select2_select.closest('table');
            if (!$table.hasClass('widefat')) {
              setTimeout(start_bulk_check, 500);
              return;
            }
            if ($select2_select.data('select2'))
            {
              var $select2_clone = $select2_select_clone.clone();
              $select2_select.select2('destroy').replaceWith($select2_clone);
              $select2_select = $select2_clone;
            }
            calculateSelected();
            calculateTotal();
            $select2_select.select2({
              ...
              templateResult: function (item, li) {
                ...
                if ($('.' + container_class + ':visible #in-' + taxonomy + '-' + item.id + ':checked').length)
                {
              ...
              var $checkbox = $('.' + container_class + ':visible #in-' + taxonomy + '-' + item.id);
              if (selecting)
              {
                if ($checkbox.length) {
              ...
              if (!is_searched) {
                ...
                $search.val($('#' + search_input_id).val()).trigger('input');
                return false;
              }
          ...
          setTimeout(start_bulk_check, 100);
        });
      } else {
        $('body').on('click', '.editinline, #doaction', function () {
          if (this.id === 'doaction')
          ...
          var $checkbox_ul = $('.' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist');
          $('li>label:visible input[type=checkbox]:checked', $checkbox_ul).each(function () {
      ...
      function calculateTotal(number) {
        if (!number)
        {
          total_visible = $('.' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist li:visible>label:visible').length;
        } else {
        ...
        $('#' + total_count_id).html(lang.total_items + '<span>' + total + '</span>');
        $('#' + results_count_id).html(lang.total_results + '<span>' + total_visible + '</span>');
      }
      var calculate_selected_timer;
      function calculateSelected() {
        clearTimeout(calculate_selected_timer);
        calculate_selected_timer = setTimeout(function () {
          var total = $('.' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist li[data-selected="1"]').length;
          $('#' + selected_count_id).html(lang.total_selected + '<span>' + total + '</span>');
        }, 500);
      }
      var updateStyle = function (event) {
        var $checkbox_container = $('.' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist');
        $checkbox_container.addClass('reducing_results');
        clearTimeout(search_style_timer);
        var new_value = stripDiacritics($('#' + search_input_id).val());
        var show_count = $('#' + search_check_count_id).is(':checked');
        var show_cnt = $('#' + search_check_cnt_id).is(':checked');
        var show_select = tax_options.select2 || (!tax_options.select2 && $('#' + search_check_select_id).is(':checked'));
        search_style_timer = setTimeout(function () {
          $('#' + search_style_id).empty();
          if (new_value.length || show_count || show_select || show_cnt) {
            ...
            if (new_value.length) {
              ...
              for (var i = 0; i < val_split.length; i++)
              {
                ...
                if (item.length)
                {
                  css += '.' + container_class + ' .cat-checklist.' + taxonomy + '-checklist li' + (not ? ':not(' : '') + '[data-' + attr_selector + operator + '="' + item + '"]' + (not ? ')' : '') + '>label{display:none;}';
                }
              }
            }
            if (show_cnt) {
              css += '.' + container_class + ' .cat-checklist.' + taxonomy + '-checklist li[data-cnt="0"]>label{display:none;}';
            }
            if (show_count) {
              css += '.' + container_class + ' .cat-checklist.' + taxonomy + '-checklist li:not([data-count="0"])>label{display:none;}';
            }
            if (show_select) {
              css += '.' + container_class + ' .cat-checklist.' + taxonomy + '-checklist li:not([data-selected="1"])>label{display:none;}';
            }
            $('#' + search_style_id).html(css);
          }
          calculateTotal();
          var $checkbox_container = $('.' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist');
      ...
      $('body').on('input', '#' + search_input_id, updateStyle);
      $('body').on('change', '#' + search_check_cnt_id, updateStyle);
      $('body').on('change', '#' + search_check_count_id, updateStyle);
      if (!tax_options.select2)
      {
        $('body').on('change', '#' + search_check_select_id, updateStyle);
        $search_check_select.on('change', updateStyle);
        $('body').on('change', '#' + search_check_all_id, function () {
          var $checkbox_container = $('.' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist');
      ...
      $('body').on('change', '#' + search_uncheck_all_id, function () {
        var $checkbox_container = $('.' + container_class + ':visible .cat-checklist.' + taxonomy + '-checklist');
